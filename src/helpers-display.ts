...
function color(config: CardConfig, name?: string): string {
  ...
  return config.color_config[name ?? 'default_inactive'] ?? '';
}
    ...
    <tr>
      ...
      ${row.cells.map((cell, n) => {
          ...
          if (cell.cellActive) {
            ...
            bgColor = color(config, col.active_color ?? 'default_active')

          } else if (cell.isActiveTime || cell.isActiveCost) {
            // The cell might be within a timeslot which doesn't match cost, so we can show 'off' but still inactive
            if (col.inactive_text)
              text = col.inactive_text;
            else
              text = localize('states.inactive');

            bgColor = color(config, col.inactive_color ?? 'default_inactive');
          } else {
            bgColor = '';
            text = '';
          }
      ...
      <td style="color: ${color(config, row.totalPower > 0 ? 'high' : 'negative')}">
        ${(row.totalPower / 1000).toFixed(config.power_decimals)}kW
      </td>
      <td style="color: ${color(config, row.cost > 0 ? 'high' : 'negative')}">
        ${row.cost.toFixed(config.price_decimals)}${config.price_unit}
      </td>
    </tr>`
}
